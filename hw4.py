...
cross_valid = False
coding_dim_vec = [2, 4, 6, 8]
lrate_vec = [ 0.001, 0.00001]
batch_size_vec = [ 256, 512, 1024]
num_epochs = 20
loss_tresh = 0.25
min_loss = 1000
net_number = 0
best_net_number = 0
if cross_valid:
    for coding_dim in coding_dim_vec:
        for lrate in lrate_vec:
            for batch_size in batch_size_vec:
                net = Autoencoder(coding_dim)
                net.to(device)
                optim = torch.optim.Adam(net.parameters(), lr=lrate,  weight_decay=1e-5)
                print("\t net number: " + str(coding_dim) + " " + str(lrate) +" "+str(batch_size))
                print("net param: " + str(net_number+1))
                net_number = net_number+1
                kfold = create_kFold(train_dataset, batch_size);
                loss_record = []
                not_penalized = True
                
                for train_set, val_set in kfold:
                    if(not_penalized):
                        for epoch in range(num_epochs):
                            train_epoch(net, train_set, loss_fn, optim)
                        val_loss = test_epoch(net, val_set, loss_fn, optim)
                        if(val_loss < loss_tresh):
                            loss_record.append(val_loss)
                        else:
                            print("Net exceed limit ")
                            loss_record.append(1000)
                            not_penalized = False
                            break
                loss_for_set_param = np.mean(loss_record)
                if(loss_for_set_param < min_loss):
                    best_param = [coding_dim, lrate, batch_size]
                    min_loss = loss_for_set_param
                    best_net_number = net_number
                    print("temp best loss: " + str(min_loss))
    print("\t net opt is: " + str(best_param))
    print("\t net opt is number : " + str(best_net_number+1))

#%% best param 8, 1e-03, 256
...
for epoch in range(num_epochs_final):
        ...
        torch.save(net.state_dict(), 'net_params_final.pth')
